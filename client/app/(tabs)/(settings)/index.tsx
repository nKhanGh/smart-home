...
export default function SettingsScreen() {
    ...
    DeviceService.getThresholdDevices()
      .then(res => {
        ...
        res.data.forEach((d: any) => {
          initial[d.type] = typeof d.threshold === 'number' ? d.threshold : Number(d.threshold);
        });
  ...
  const handleThresholdChange = async (deviceId: string, value: number, deviceType: string) => {
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.userCardGreen}>
          <View style={styles.avatarBig}>
            <Text style={styles.avatarBigText}>{user?.avatarInitials || 'U'}</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.userNameGreen}>{user?.fullName || 'Chưa đăng nhập'}</Text>
            <Text style={styles.userRoleGreen}>{user?.role === 'admin' ? '👑 Quản trị viên' : '👤 Thành viên'}</Text>
        ...
        <TouchableOpacity style={styles.sectionCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sectionSubText}>Thêm, xóa, phân quyền người dùng</Text>
          </View>
        ...
        {loading ? (
          ...
          <>
            {['temperatureSensor', 'lightSensor'].map((typeKey) => {
              ...
              let icon = '🔆', label = device.name, color = '#22C55E', unit = '', min = 0, max = 100;
              ...
              if (typeof sliderValue !== 'number' || isNaN(sliderValue)) {
                sliderValue = (typeof device.threshold === 'number' && !isNaN(device.threshold))
                  ? device.threshold
                  : min;
              }
              return (
                <View style={styles.sectionCardAlert} key={device.type}>
                  <View style={styles.sectionAlertRow}>
                    ...
                    <Text style={styles.thresholdValue}>{typeof sliderValue === 'number' && !isNaN(sliderValue) ? sliderValue : '--'} {unit}</Text>
                  </View>
                  <Slider
                    ...
                    value={typeof sliderValue === 'number' && !isNaN(sliderValue) ? sliderValue : min}
                    step={1}
                    onValueChange={v => handleSliderValueChange(device.type, v)}
                    onSlidingComplete={v => handleThresholdChange(device._id, v, device.type)}
                    disabled={updating === device.type}
                  />
                  <View style={styles.thresholdMinMaxRow}>
                    <Text style={styles.thresholdMinMax}>{min}{unit}</Text>
                    <Text style={styles.thresholdMinMax}>{max}{unit}</Text>
                  </View>
                </View>
              );
            })}
        ...
        <TouchableOpacity style={styles.sectionCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sectionSubText}>Xem lịch sử bật, tắt thiết bị</Text>
          </View>
          <Icon name="angle-right" size={20} color="#A0A0A0" />
        </TouchableOpacity>
